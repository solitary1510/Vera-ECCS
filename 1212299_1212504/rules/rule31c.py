    ...
    for line in lines:
        ...
        if iterator != None:
            for mem in iterator:
                ...
                if line[mem.start() - 1] != ' ':
                    vera.report (filename, lineCounter, ''.join(['Operator shall always be preceded and followed by one space at position ', str(mem.start() + 1)]));
                else: # Sau, kiem tra dau bang => khong co dau cach => bao loi
                    if (line[mem.start() + 1] == '=') and (line[mem.start() + 2] != ' '):
                        vera.report (filename, lineCounter, ''.join(['Operator shall always be preceded and followed by one space at position ', str(mem.start() + 2)]));
